...
//pthread_cond_t condition_incr = PTHREAD_COND_INITIALIZER;
//pthread_cond_t condition_decr = PTHREAD_COND_INITIALIZER;
...
void* print_thread(void* arg) {
    while (1) {
        ...
        //pthread_cond_wait(&condition_incr, &mutex);
        //pthread_cond_wait(&condition_decr, &mutex);
        pthread_mutex_lock(&mutex);
        printf("x = %d\n", x);
        pthread_mutex_unlock(&mutex);
    }
